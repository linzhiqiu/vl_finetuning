...
if EVAL_MODE == 'linear':
    #### Partial START
    EVAL_DIR = "./eval_testset_zeroshot"
    ...
    DATASETS_TO_TESTSET = {
        'imagenet': ['imagenet_a', 'imagenet_r', 'imagenet_sketch', 'imagenetv2', 'imagenet'],
...
def take_average(all_seed_dict,
    ...
    columns.append([best_lr, best_wd, best_batchsize, best_iters, best_key,
                    ...
                    avg_dict[best_lr][best_wd][best_batchsize][best_iters][best_key]['logit_scale'], std_dict[best_lr][best_wd][best_batchsize][best_iters][best_key]['logit_scale'],
                    'head',
...
def take_extra_test_average(all_extra_test_dict):
    ...
    for dataset in ALL_DATASETS:
        avg_dict[dataset] = np.mean([all_extra_test_dict[seed][dataset] for seed in ALL_SEEDS])
        std_dict[dataset] = np.std([all_extra_test_dict[seed][dataset] for seed in ALL_SEEDS])
                                            ...
                                            if all_seed_finished:
                                                ...
                                                for seed in SEEDS:
                                                    ...
                                                    head, num_classes, in_features = make_classifier_head(
                                                        "linear_zeroshot_norm", cfg.FEATURE.BACKBONE, cfg.ARCHITECTURE.BIAS, text_dataset)
                                                    ...
                                                    eval_heads = get_eval_heads(deepcopy(old_logit_head.head))
                                                    assert len(eval_heads) == 1
                                                    ...
                                                    extra_test_path = os.path.join(checkpoint_dir, "extra_test_zeroshot.pth")
                                                    ...
                                                    if False:
                                                        ...
                                                        for test_dataset_name in DATASETS_TO_TESTSET[dataset]:
                                                            ...
                                                            if label_map is None:
                                                                ...
                                                                extra_test_dict[test_dataset_name] = test_acc
                                                            else:
