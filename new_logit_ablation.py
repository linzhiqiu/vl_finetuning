...
def get_logit_name(cfg):
    name = "logit_"
    assert not cfg.LOGIT.HEAD_NORM
    if cfg.LOGIT.FEATURE_NORM:
        name += "feature_norm"
    if cfg.LOGIT.USE_LOGIT_SCALE:
        name += f"_fixed_logit_scale_{cfg.LOGIT.LOGIT_SCALE}"
    elif cfg.LOGIT.LEARN_LOGIT_SCALE:
        name += f"_learn_logit_scale_from_{cfg.LOGIT.INIT_LEARN_LOGIT_SCALE}"
    return name
