...
def main():
    ...
    for dataset_idx, dataset in enumerate(CLASS_MAP):
        ...
        crossmodal_features = text_features['features']
        crossmodal_labels = text_features['labels']
        ...
        for shots in [1, 2, 4]:
            for architecture, cross_modal in [('linear_zeroshot', 'normtext_ratio_0.5')]:
                if True:
                    ...
                    save_dir = os.path.join(dataset_dir, f'{shots}_shot_{architecture}_{cross_modal}_textonly')
                    makedirs(save_dir)
