...
def get_image_feature_name(cfg):
    ...
    image_feature_name = "-".join([get_view_name(cfg), 'plus_ccrop', f"imlayer_{cfg.FEATURE.LAYER_IDX}"])
...
def get_logit_name(cfg):
    name = "logit_"
    name += f"_f_{cfg.LOGIT.FEATURE_NORM}_h_{cfg.LOGIT.HEAD_NORM}"
    if cfg.LOGIT.USE_LOGIT_SCALE:
        name += f"_fixedlogit_{cfg.LOGIT.LOGIT_SCALE}"
    elif cfg.LOGIT.LEARN_LOGIT_SCALE:
        name += f"_learnlogit_{cfg.LOGIT.INIT_LEARN_LOGIT_SCALE}"
    return name
...
def main(args):
    ...
    ccrop_features_path = os.path.join(
    ...
    image_val_dataset = TensorDataset(
        ccrop_features['val']['features'], ccrop_features['val']['labels'])
        ...
        for wd in cfg.OPTIM.WEIGHT_DECAY:
            for batch_size in VALID_BATCH_SIZES:
                for iters in cfg.OPTIM.MAX_ITER:
                    ...
                    train_result_dict = {
                        'best_val': best_val_dict,
                        'last_iter': last_iter_dict,
                    }
                    test_result_dict = {
                        'best_val': {},
                        'last_iter': {},
                    ...
                    for key in ['best_val', 'last_iter']:
                        result_dict = train_result_dict[key]
