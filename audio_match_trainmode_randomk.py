...
EVAL_DIR = "./audio_clip_match_random_trainmode/"
...
def main():
    ...
    for dataset_idx, dataset in enumerate(MAP):
        ...
        for text_idx in MAP[dataset]:
            ...
            k = 10
            perm = torch.randperm(logits_text_audio.shape[1])[:k]
            randomk_score = logits_text_audio[0, perm]
            perm = perm.tolist()
            randomk_indexs = [audio_features_indices[idx] for idx in perm]
            result = {
                'mode': 'text_to_randomk_audio',
                'class': class_idx_to_text[text_idx],
                'class_idx': text_idx,
            }
            for k_idx, (score, audio_idx) in enumerate(zip(randomk_score, randomk_indexs)):
                result[f'rand{k_idx}_score'] = f"{score.item():.3f}"
                result[f'rand{k_idx}_audio'] = class_idx_to_audio[int(audio_features['labels'][int(audio_idx)])]
            for k_idx, (score, audio_idx) in enumerate(zip(randomk_score, randomk_indexs)):
                result[f'rand{k_idx}_file'] = f"{audio_filenames[audio_idx]}"
        ...
        # save results as csv file with header
        df = pd.DataFrame(results)
        df.to_csv(os.path.join(result_dir, f"{dataset}_text_randk_audio.csv"), index=False, header=True)
        print(f"Saved results to {os.path.join(result_dir, f'{dataset}_text_randk_audio.csv')}")
