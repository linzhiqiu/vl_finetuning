...
EVAL_DIR = "./audio_few_shot_sam_results/"

ESC_DIR = "/data3/zhiqiul/vl_finetuning/data/esc-50/ESC-50-master/"
AUDIO_FEATURES_PATH = f"{ESC_DIR}features.pt"
...
def extract_audio_features(sorted_classes, class_map):
    ...
    sam_features = sam_features / sam_features.norm(dim=-1, keepdim=True)
    features_dict['features'] = sam_features
    features_dict['labels'] = torch.Tensor([]).long()
    for label in sam_labels:
        has_found = False
        for class_idx in class_map:
            if class_map[class_idx] == label:
                features_dict['labels'] = torch.cat((features_dict['labels'], torch.Tensor([sorted_classes.index(class_idx)]).long()))
                has_found = True
                break
        if not has_found:
