...
metadata = read_csv(f'{PATH}/meta/esc50.csv')
...
if not os.path.exists(save_path):
    ...
    for idx, line in tqdm(enumerate(metadata[1:])):
        path_to_audio = f'{PATH}/ESC-50-master/audio/{line[FILE_PATH_INDEX]}'
        fold_index = int(line[FOLD_INDEX])
    ...
    for fold_index in range(5):
        ...
        esc50_dict['features'] = final_audio_stacked
        esc50_dict['labels'] = torch.Tensor([d['class'] for d in meta[fold_index]]).long()
        esc50_dict['labelnames'] = [d['classname'] for d in meta[fold_index]]
        esc50_dict['paths'] = [d['path'] for d in meta[fold_index]]
