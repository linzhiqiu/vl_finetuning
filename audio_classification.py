...
EVAL_DIR = "./audio_clip_results/"

AUDIO_FEATURES_PATH = "/data3/zhiqiul/vl_finetuning/data/esc-50/ESC-50-master/features.pt"
...
def main():
    ...
    for dataset_idx, dataset in enumerate(DATASETS):
        ...
        # find sam feature's filepaths
        sam_filepaths = {}
        for sam_idx, feature in enumerate(text_features['sam_audio_features']):
            smallest = None
            label = class_idx_to_audio[sam_idx]
            for idx, my_feature in enumerate(audio_features['features']):
                error = (feature-my_feature).sum().abs()
                if smallest is None or error < smallest[0]:
                    smallest = (error, idx)
                if error < 0.5:
                    sam_filepaths[label] = audio_filenames[idx]
                    break
            else:
                print(f"Couldn't find feature for label {label}")
        ...
        logits_text_audio = scale_audio_text * \
            text_features['features'] @ audio_features['features'].T
        topk_scores, topk_indices = torch.topk(logits_text_audio, k=20, dim=1)
        # import pdb; pdb.set_trace()
        # confidence_text_audio = torch.softmax(logits_text_audio, dim=1)
        # score_text_audio, pred_text_audio = torch.max(confidence_text_audio, dim=1)
        results = list()
        for idx, (topk_score, topk_indexs) in enumerate(zip(topk_scores, topk_indices)):
            result = {
                'mode': 'text_to_topk_audio',
                'class': class_idx_to_text[idx],
                'class_idx': idx,
                # 'audio': class_idx_to_audio[int(audio_features['labels'][int(pred)])],
                # 'audio_idx': int(pred),
                # 'audio_file': audio_filenames[int(pred)]
            }
